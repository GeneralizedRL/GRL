...
def load_triples(data_path, entity_index_path, relation_index_path, group_examples_by_query=False,
    ...
    # def triple2ids(e1, e2, r,lenmask):
    #     return entity2id[e1], entity2id[e2], relation2id[r],lenmask
    def triple2ids(e1, e2, r):
        return entity2id[e1], entity2id[e2], relation2id[r]
    ...
    with open(data_path) as f:
        ...
        for line in f:
            ...
            if group_examples_by_query:
                ...
                # triples.append(triple2ids(e1, e2, r, lenmask))
                triples.append(triple2ids(e1, e2, r))
                if add_reverse_relations:
                    # triples.append(triple2ids(e2, e1, r + '_inv',lenmask))
                    triples.append(triple2ids(e2, e1, r + '_inv'))
    if group_examples_by_query:
        for e1_id in triple_dict:
            for r_id in triple_dict[e1_id]:
                # triples.append((e1_id, list(triple_dict[e1_id][r_id]), r_id,lenmask))
...
def load_triples_groupbyrelation(data_path, entity_index_path, relation_index_path, group_examples_by_query=False,
                                 add_reverse_relations=False, seen_entities=None, verbose=False):
    """
    Convert triples stored on disc into indices.
    """
    ...
    for one in triples:
        one_id = one[2]
        if one_id not in new_triples:
            new_triples[one_id] = []
        new_triples[one_id].append(one)
    a = sorted(new_triples.items(), key=lambda x: x[0], reverse=True)
    return a
...
def prepare_kb_envrioment(raw_kb_path, train_path, dev_path, test_path, add_reverse_relations=False):
    """
    ...
    print(len(raw_kb_triples))

    list2 = [i for i in raw_kb_triples if i not in dev_triples]
    print(len(list2))
    raw_kb_triples = [i for i in list2 if i not in test_triples]
    print(len(raw_kb_triples))

    train_path2 = train_path
    dirname, filename = os.path.split(train_path)

    new_file = os.path.join(dirname, "train_ini.triples")
    os.rename(train_path, new_file)

    with open(train_path2, 'w') as f:
        for pertriple in set(raw_kb_triples + train_triples):
            ttt = pertriple.strip().split()
            if len(ttt) != 3: print(pertriple)
            e1 = ttt[0]
            e2 = ttt[1]
            r = ttt[2]
            str0 = e1 + '\t' + e2 + '\t' + r + '\n'
    ...
    for line in set(raw_kb_triples + train_triples + dev_triples + test_triples):
        ttt = line.strip().split()
        e1 = ttt[0]
        e2 = ttt[1]
        r = ttt[2]
